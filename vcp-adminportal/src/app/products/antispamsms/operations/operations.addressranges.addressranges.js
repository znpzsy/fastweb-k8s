(function () {
    ...
    AntiSpamSMSOperationsAddressRangesMsisdnAddressRangesModule.controller('AntiSpamSMSOperationsAddressRangesMsisdnAddressRangesCtrl', function ($scope, $uibModalInstance, $log, $filter, $uibModal, $translate, notification, NgTableParams, NgTableService,
        ...
        tableScope.$watch('newAddressRange.start', function (newVal) {
            if (newVal && tableScope.newAddressRange) {
                var foundItem = _.find($scope.addressRangeList.list, function (item) {
                    return (String(item.start) === String(newVal) && String(item.end) === String(tableScope.newAddressRange.end));
                });

                tableScope.form.start.$setValidity('availabilityCheck', _.isUndefined(foundItem));
                tableScope.form.end.$setValidity('availabilityCheck', true);
            }
        });
        tableScope.$watch('newAddressRange.end', function (newVal) {
            if (newVal && tableScope.newAddressRange) {
                var foundItem = _.find($scope.addressRangeList.list, function (item) {
                    return (String(item.start) === String(tableScope.newAddressRange.start) && String(item.end) === String(newVal));
                });

                tableScope.form.start.$setValidity('availabilityCheck', true);
                tableScope.form.end.$setValidity('availabilityCheck', _.isUndefined(foundItem));
            }
        });
