(function () {
    ...
    angular.module('adminportal.products.antispamsms.operations.contentfilters.mosmscontent', []);

    var AntiSpamSMSOperationsContentFiltersMOSMSContentModule = angular.module('adminportal.products.antispamsms.operations.contentfilters.mosmscontent');
    ...
    var moSMSContentListStateDef = function (participant) {
        return {
            url: "/list",
            templateUrl: "products/antispamsms/operations/operations.contentfilters.html",
            controller: 'AntiSpamSMSOperationsContentFiltersMOSMSContentCtrl',
            data: prepareData(participant),
            resolve: {
                moSMSContentFilters: ['SMSAntiSpamConfigService', function (SMSAntiSpamConfigService) {
                    return SMSAntiSpamConfigService.getContentFiltersList(direction, participant);
                }]
            }
        };
    };
    var moSMSContentNewStateDef = function (participant) {
        return {
            url: "/new",
            templateUrl: "products/antispamsms/operations/operations.contentfilters.detail.html",
            controller: 'AntiSpamSMSOperationsContentFiltersNewMOSMSContentCtrl',
            data: prepareData(participant)
        };
    };
    var moSMSContentUpdateStateDef = function (participant) {
        return {
            url: "/update/:name",
            templateUrl: "products/antispamsms/operations/operations.contentfilters.detail.html",
            controller: 'AntiSpamSMSOperationsContentFiltersUpdateMOSMSContentCtrl',
            data: prepareData(participant),
            resolve: {
                moSMSContentFiltersEntry: ['$stateParams', 'SMSAntiSpamConfigService', function ($stateParams, SMSAntiSpamConfigService) {
    ...
    AntiSpamSMSOperationsContentFiltersMOSMSContentModule.config(function ($stateProvider) {

        $stateProvider.state('products.antispamsms.operations.contentfilters.mosmscontent', {
            abstract: true,
            url: "/mosmscontent",
            template: '<div ui-view></div>',
            data: {
                pageHeaderKey: 'Products.AntiSpamSMS.Operations.ContentFilters.MOSMSContent.Title'
            }
        }).state('products.antispamsms.operations.contentfilters.mosmscontent.aparty', {
            abstract: true,
            url: "/aparty",
            template: '<div ui-view></div>',
            data: {
        ...
        // AParty
        $stateProvider.state('products.antispamsms.operations.contentfilters.mosmscontent.aparty.list', moSMSContentListStateDef('A_PARTY')
        ).state('products.antispamsms.operations.contentfilters.mosmscontent.aparty.new', moSMSContentNewStateDef('A_PARTY')
        ).state('products.antispamsms.operations.contentfilters.mosmscontent.aparty.update', moSMSContentUpdateStateDef('A_PARTY'));

    });

    AntiSpamSMSOperationsContentFiltersMOSMSContentModule.controller('AntiSpamSMSOperationsContentFiltersMOSMSContentCommonCtrl', function ($scope, $log, $state, STATES, SMS_ANTISPAM_RANGE_POLICIES, SMS_ANTISPAM_EVALUATION_TYPES, SMS_ANTISPAM_CASE_SENSITIVITY,
                                                                                                                                            SMS_ANTISPAM_REJECT_METHODS_3) {
        $log.debug('AntiSpamSMSOperationsContentFiltersMOSMSContentCommonCtrl');

        var participantStateKey = $state.current.data.participantStateKey;

        $scope.listState = "products.antispamsms.operations.contentfilters.mosmscontent." + participantStateKey + ".list";
        $scope.newState = "products.antispamsms.operations.contentfilters.mosmscontent." + participantStateKey + ".new";
        $scope.updateState = "products.antispamsms.operations.contentfilters.mosmscontent." + participantStateKey + ".update";
    ...
    AntiSpamSMSOperationsContentFiltersMOSMSContentModule.controller('AntiSpamSMSOperationsContentFiltersMOSMSContentCtrl', function ($scope, $log, $controller, $uibModal, $filter, $translate, notification, NgTableParams, NgTableService,
                                                                                                                                      SMSAntiSpamConfigService, moSMSContentFilters) {
        $log.debug('AntiSpamSMSOperationsContentFiltersMOSMSContentCtrl');

        $controller('AntiSpamSMSOperationsContentFiltersMOSMSContentCommonCtrl', {$scope: $scope});
        ...
        var contentFilters = moSMSContentFilters ? moSMSContentFilters : [];

        // MO SMS Content Filter list
        ...
        // END - MO SMS Content Filter list
        ...
        $scope.remove = function (entry) {
            ...
            modalInstance.result.then(function () {
                $log.debug('Removing mo sms content filters entry: ', entry);

                SMSAntiSpamConfigService.deleteContentFiltersEntry($scope.direction, $scope.participant, entry.name).then(function (response) {
                    $log.debug('Removed mo sms content filters entry: ', entry, ', response: ', response);
                    ...
                    $log.debug('Cannot delete mo sms content filters entry: ', entry, ', response: ', response);
                });
            });
        };
    ...
    AntiSpamSMSOperationsContentFiltersMOSMSContentModule.controller('AntiSpamSMSOperationsContentFiltersNewMOSMSContentCtrl', function ($scope, $log, $controller, $state, $translate, notification, SMSAntiSpamConfigService) {
        $log.debug('AntiSpamSMSOperationsContentFiltersNewMOSMSContentCtrl');
    ...
    AntiSpamSMSOperationsContentFiltersMOSMSContentModule.controller('AntiSpamSMSOperationsContentFiltersUpdateMOSMSContentCtrl', function ($scope, $log, $controller, $state, $translate, notification, SMSAntiSpamConfigService, moSMSContentFiltersEntry) {
        $log.debug('AntiSpamSMSOperationsContentFiltersUpdateMOSMSContentCtrl');

        $controller('AntiSpamSMSOperationsContentFiltersMOSMSContentCommonCtrl', {$scope: $scope});

        $scope.entry = moSMSContentFiltersEntry;
        ...
        $scope.save = function (entry) {
            ...
            SMSAntiSpamConfigService.updateContentFiltersEntry($scope.direction, $scope.participant, entryItem).then(function (response) {
                $log.debug('Updated mo sms content filters entry: ', entryItem, ', response: ', response);
                ...
                $log.debug('Cannot update mo sms content filters entry: ', entryItem, ', response: ', response);
            });
        };
    });
