(function () {
    ...
    angular.module('adminportal.products.antispamsms.operations.screenings.applicationmt.smscgt', []);

    var AntiSpamSMSOperationsScreeningsApplicationMTSMSCGTModule = angular.module('adminportal.products.antispamsms.operations.screenings.applicationmt.smscgt');

    AntiSpamSMSOperationsScreeningsApplicationMTSMSCGTModule.controller('AntiSpamSMSOperationsScreeningsApplicationMTSMSCGTCtrl', function ($scope, $uibModalInstance, $log, $filter, $uibModal, $translate, notification, NgTableParams, NgTableService,
                                                                                                                                            STATES, SMSAntiSpamConfigService, smscGTList, entry) {
        $log.debug("AntiSpamSMSOperationsScreeningsApplicationMTSMSCGTCtrl");

        smscGTList = smscGTList ? smscGTList : [];
        smscGTList = $filter('orderBy')(smscGTList, 'smscGT');
        ...
        $scope.addNewSMSCGT = function (smscGT) {
            ...
            SMSAntiSpamConfigService.createApplicationMTFilteringSMSCGT($scope.entry.senderAddressRangeStart, $scope.entry.senderAddressRangeEnd, smscGTItem).then(function (response) {
                if (response && response.value === "ALREADY_SUBSCRIBED") {
                    $log.debug('Cannot add smsc gt: ', smscGTItem, ', response: ', response);
                    ...
                    $log.debug('Added smsc gt: ', smscGTItem);
        ...
        $scope.removeSMSCGT = function (smscGTItem) {
            ...
            modalInstance.result.then(function () {
                $log.debug('Removing smsc gt: ', smscGTItem);

                SMSAntiSpamConfigService.deleteApplicationMTFilteringSMSCGTList($scope.entry.senderAddressRangeStart, $scope.entry.senderAddressRangeEnd, smscGTItem.smscGT).then(function (response) {
                    $log.debug('Deleted smsc gt: ', smscGTItem, ', response: ', response);
                    ...
                    $log.debug('Cannot delete smsc gt: ', smscGTItem, ', response: ', response);
                });
            });
        };
