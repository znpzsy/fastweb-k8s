(function () {
    ...
    ApplicationServices.factory('UtilService', function ($window, $log, $translate, notification, $timeout, DURATION_UNITS,
        ...
        return {
            COUNTRY_CODE: "39",
            SESSION_KEY: '_it_fw_vco_a_sk',
            USERNAME_KEY: '_it_fw_vco_a_un',
            SITE_INFORMATION_KEY: '_it_fw_vco_a_si',
            SITE_CONFIGURATION_KEY: '_it_fw_vco_a_sc',
            LATEST_STATE: '_it_fw_vco_a_lst',
            USER_RIGHTS: '_it_fw_vco_a_ur',
            USER_ORGANIZATION_KEY: '_it_fw_vco_a_uok',
            USER_ORGANIZATION_ID_KEY: '_it_fw_vco_a_uoik',
            USER_ORGANIZATION_NAME_KEY: '_it_fw_vco_a_onk',
            // Created with this command: node -e "console.log(require('crypto').randomBytes(32).toString('hex'));"
            FREDERIC_CHOPIN: "90f6afc1702366de05916521a572f361a8c7a391f85b55f6f3e34331db999788",
            Validators: {
                Msisdn: /^[0-9]{0,15}$/,
                InternationalPhoneNumber: /^(\+){0,1}[0-9]{0,15}$/,
                ValidPhoneNumber: /^(39){1}[0-9]{0,15}$/,
    ...
    ApplicationServices.factory('CMPFService', function ($log, $q, $filter, notification, $translate, DateTimeConstants, UtilService, CMPFAuthRestangular,
        ...
        return {
            ...
            getUserProfile: function (userAccount) {
                var userProfileDefn = _.findWhere(userAccount.profiles, {profileDefinitionName: this.USER_PROFILE_NAME});

                return userProfileDefn;
            },
    ...
    ApplicationServices.factory('SMSAntiSpamConfigService', function ($log, $timeout, SMSAntiSpamConfigRestangular, UtilService) {
        return {
            ...
            getBlackLists: function (list, key) {
                var promise = SMSAntiSpamConfigRestangular.one('/' + list + '-blacklist/pattern/' + key).get();
                UtilService.addPromiseToTracker(promise);

                return promise;
            },
            getBlackListEntry: function (list, key) {
                var promise = SMSAntiSpamConfigRestangular.one('/' + list + '-blacklist/' + list + '/' + key).get();
                UtilService.addPromiseToTracker(promise);

                return promise;
            },
            createBlackListEntry: function (list, entry) {
                var promise = SMSAntiSpamConfigRestangular.all('/' + list + '-blacklist/insert').post(entry);
                UtilService.addPromiseToTracker(promise);

                return promise;
            },
            updateBlackListEntry: function (list, entry) {
                var promise = SMSAntiSpamConfigRestangular.all('/' + list + '-blacklist/update').customPUT(entry);
                UtilService.addPromiseToTracker(promise);

                return promise;
            },
            deleteBlackListEntry: function (list, key) {
                var promise = SMSAntiSpamConfigRestangular.one('/' + list + '-blacklist/' + list + '/' + key).remove();
                UtilService.addPromiseToTracker(promise);
    ...
    ApplicationServices.factory('DiagnosticsService', function ($log, DiagnosticsRestangular, UtilService) {
        return {
            ...
            queryHostStats: function (query, promiseTracker) {
                var promise = DiagnosticsRestangular.all('hoststats/_search').post(query);
                UtilService.addPromiseToTracker(promise, promiseTracker);
                return promise;
            }
        };
    ...
    ApplicationServices.factory('GeneralESService', function ($log, $filter, UtilService, SessionService, ESClient, ESClientRemote,
                                                              ...
                                                              SMSAntiSpamESClient, SMSAntiSpamESClientRemote, SMSAntiSpamESAdapterClient,
                                                              SMSAntiSpamESAdapterClientRemote, ServerConfigurationService) {
    ...
    ApplicationServices.factory('PentahoApiService', function ($q, $log, $filter, $locale, PentahoApiRestangular, UtilService, DateTimeConstants, DAYS_OF_WEEK) {
        ...
        return {
            ...
            // Restful API Methods
            healthCheck: function (promiseTracker) {
                var promise = PentahoApiRestangular.one('mantle/isAuthenticated').get();
                UtilService.addPromiseToTracker(promise, promiseTracker);
                ...
                promise.then(function (response) {
                    ...
                    $timeout(function () {
                        PentahoApiRestangular.one('mantle/isAuthenticated').get().then(function (response) {
                            deferred.resolve(response);
                        });
                    }, 1000);
                });
