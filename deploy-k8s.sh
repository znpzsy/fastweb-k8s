...
REPOSITORY="consolportals-it-fastweb"
K8MANIFEST="consolportals_it_fastweb_vcp"
SITE="vcp"
NAMESPACE="fastweb-vcp-services"
PORTALS=("adminportal" "ccportal")
ALL_SERVICES=("httpd" "a3gw")
CONF_DIR="a3gw/vcp/conf.k8s"
A3GW_CONFIGMAP="consolportals-it-fastweb-vcp-a3gw-conf"
A3GW_SECRET="consolportals-it-fastweb-vcp-a3gw-conf-secret"
...
#      -n "$NAMESPACE" &>/dev/null
    ...
    kubectl rollout restart deployment consolportals-it-fastweb-vcp-a3gw-deployment -n "$NAMESPACE" &>/dev/null || true
    kubectl rollout status  deployment consolportals-it-fastweb-vcp-a3gw-deployment -n "$NAMESPACE" --timeout=120s &>/dev/null || true
...
apply_ingress() {
    ...
    if [[ "$mode" == "httpd" ]]; then
        ...
        if kubectl apply -f "k8s/consolportals-it-fastweb-vcp-httpd-ingress.yaml" -n "$NAMESPACE" &>/dev/null; then
        ...
        if kubectl apply -f "k8s/consolportals_it_fastweb_ingress.yaml" -n "$NAMESPACE" &>/dev/null; then
