(function () {
    ...
    CCPortalApplicationModule.run(function ($rootScope, $log, $timeout, $translate, $document, $http, notification, Restangular, SessionService, $sce, $state, $stateParams,
                                            IdleServiceFactory, UtilService, CMPFCacheService, AuthorizationService, DateTimeConstants, CURRENCY,
                                            CMPFService, DEFAULT_REST_QUERY_LIMIT, CCPortalMainPromiseTracker) {
        ...
        $rootScope.isAdminUser = (typeof isAdminUserKey === 'boolean' ? isAdminUserKey : false);
        ...
        if (SessionService.isSessionValid()) {
            ...
            CMPFService.getUserAccountGroups($rootScope.systemUserId, false, false).then(function (accountGroupsResponse) {
                ...
                var userBMSAdminGroup = _.findWhere(accountGroupsResponse.userGroups, {name: CMPFService.VCP_BMS_ADMIN_GROUP});
        ...
        $rootScope.getSubscriberState = function () {
            ...
            return subscriberProfile.state.currentState;
        };
        ...
        $rootScope.isSubscriberActive = function () {
            ...
            return $rootScope.isSubscriberFound() ? subscriberProfile.state.currentState === 'ACTIVE' : false;
        };
                            ...
                            if (!_.isEmpty(adminPortalPermissions)) {
                                ...
                                if (isPermitted) {
                                    if (!fromParams.doNotQuerySubscriberAtStateChange && !toState.data.doNotQuerySubscriberAtStateChange &&
                                        (!toState.url.startsWith('/dashboard') && !fromState.url.startsWith('/dashboard'))) {
                                        //CMPFCacheService.getSubscriber(msisdn).then(function (response) {
    ...
    CCPortalApplicationModule.config(function ($translateProvider, RestangularProvider, $urlRouterProvider, $logProvider, $qProvider,
        ...
        IdleProvider.setLocalStorageKey('vcp-ccp-fastweb-it');
        ...
        $translateProvider.storagePrefix('VCP_CCP_FASTWEB_IT_');
