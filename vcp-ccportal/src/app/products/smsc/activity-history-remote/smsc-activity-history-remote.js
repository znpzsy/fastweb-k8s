(function () {
    ...
    angular.module('ccportal.products.smsc.activity-history-remote', [
        'ngTableExport'
    ]);

    var SmscActivityHistoryRemoteModule = angular.module('ccportal.products.smsc.activity-history-remote');

    SmscActivityHistoryRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('products.smsc.activity-history-remote', {
            abstract: true,
            url: "/activity-history-remote",
            templateUrl: "products/smsc/activity-history/smsc-activity-history.html"
        }).state('products.smsc.activity-history-remote.edr', {
            abstract: true,
            url: "",
            templateUrl: 'products/smsc/activity-history/smsc-activity-history.edr.html',
            controller: 'SmscActivityHistoryRemoteCtrl',
            resolve: {
        ...
        }).state('products.smsc.activity-history-remote.edr.view', {
            url: "",
            views: {
                'permanentTable': {
    ...
    SmscActivityHistoryRemoteModule.controller('SmscActivityHistoryRemoteCtrl', function ($scope, $log, $uibModal, $q, $filter, $timeout, $controller, $translate, notification, GeneralESService, SfeReportingService,
        ...
        $log.debug('SmscActivityHistoryRemoteCtrl');
        ...
        var getSummarizeCounts = function () {
            ...
            GeneralESService.getSmscPermanentCountRemote(preparedFilter.filter, preparedFilter.additionalFilterFields).then(function (response) {
                ...
                if (preparedFilter.additionalFilterFields.result === '0') {
                    result.transient = 0;
                    result.delivered = response.count;

                    deferred.resolve(result);
                } else {
                    SfeReportingService.getTransientCountRemote(preparedFilter.filter, preparedFilter.additionalFilterFields).then(function (response) {
                        ...
                        if (preparedFilter.additionalFilterFields.result === '1') {
                            result.delivered = 0;

                            deferred.resolve(result);
                        } else {
                            GeneralESService.getSmscPermanentDeliveredCountRemote(preparedFilter.filter, preparedFilter.additionalFilterFields).then(function (permanentResponse) {
                    ...
                    entireMsgContent: function ($q, $log, notification, $translate, GeneralESService) {
                        ...
                        if (edrRecord.partTotal === 1) {
                            ...
                            if ($scope.recordType = 'permanent') {
                                promise = GeneralESService.findSmscPermanentMessagePartsRemote(edrRecord.origAddress, edrRecord.destAddress, edrRecord.partRef, edrRecord.scTimestamp);
                            } else {
                                promise = SfeReportingService.findSmscTransientMessagePartsRemote(edrRecord.origAddress, edrRecord.destAddress, edrRecord.partRef);
                            }
        ...
        $controller('SmscActivityHistoryRemotePermanentCtrl', {$scope: $scope});
        $controller('SmscActivityHistoryRemoteTransientCtrl', {$scope: $scope});

        $controller('SmscActivityHistoryRemoteHistoryCtrl', {$scope: $scope});
        $controller('SmscActivityHistoryRemoteSummarizeCtrl', {$scope: $scope});
    });

    SmscActivityHistoryRemoteModule.controller('SmscActivityHistoryRemotePermanentCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams, $controller, UtilService, DateTimeConstants,
                                                                                                   GeneralESService, SmscOperationService, TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION,
                                                                                                   ReportingExportService) {
        $log.debug('SmscActivityHistoryRemotePermanentCtrl');
        ...
        var exportPermanentEdrs = function (mimeType) {
            ...
            var srcUrl = '/smsc-es-adapter-remote-rest/elastic-search-adapter/main_edr/_export';
        ...
        $scope.smscPermanentEdrList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                GeneralESService.findSmscPermanentEdrsRemote(preparedFilter.filter, preparedFilter.additionalFilterFields).then(function (response) {
        ...
        $scope.resendMessage = function (edrRecord) {
            ...
            if (edrRecord.partTotal === 1) {
                ...
                GeneralESService.findSmscPermanentMessagePartsRemote(edrRecord.origAddress, edrRecord.destAddress, edrRecord.partRef, edrRecord.scTimestamp).then(function (response) {
            ...
            deferred.promise.then(function (resendingMessage) {
                ...
                modalInstance.result.then(function (resendingMessage) {
                    ...
                    SmscOperationService.resendMessageRemote(resendingMessage).then(function () {
                        notification({
                            type: 'success',
    ...
    SmscActivityHistoryRemoteModule.controller('SmscActivityHistoryRemoteTransientCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, $sce, NgTableParams, $controller, UtilService, DateTimeConstants,
                                                                                                   SfeReportingService, SmscOperationService, TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION, ReportingExportService) {
        $log.debug('SmscActivityHistoryRemoteTransientCtrl');
        ...
        var exportTransientEdrs = function (mimeType) {
            ...
            var srcUrl = '/smsc-sfe-reporting-remote-rest/v1/export';
        ...
        $scope.smscTransientEdrList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                SfeReportingService.getEDRsRemote(preparedFilter.filter, preparedFilter.additionalFilterFields).then(function (response) {
        ...
        $scope.cancelAllFilteredMessages = function () {
            ...
            modalInstance.result.then(function (result) {
                ...
                SfeReportingService.deleteAllPendingMessagesRemote(filter, additionalFilterFields).then(function (response) {
        ...
        var cancelOrSendImmediately = function (sfeRecord, isCancel) {
            ...
            if (sfeRecord.partTotal === 1) {
                ...
                SfeReportingService.findSmscTransientMessagePartsRemote(sfeRecord.origAddress, sfeRecord.destAddress, sfeRecord.partRef).then(function (response) {
            ...
            deferred.promise.then(function (messageIds) {
                ...
                modalInstance.result.then(function (messageIds) {
                    ...
                    _.each(messageIds, function (messageId) {
                        if (isCancel) {
                            promises.push(SmscOperationService.deleteBySMSTicketRemote(messageId));
                        } else {
                            promises.push(SmscOperationService.sendNowBySMSTicketRemote(messageId));
                        }
                    });
    ...
    SmscActivityHistoryRemoteModule.controller('SmscActivityHistoryRemoteHistoryCtrl', function ($scope, $log, $filter, notification, $translate, $uibModal, NgTableParams, DateTimeConstants) {
        $log.debug('SmscActivityHistoryRemoteHistoryCtrl');
        ...
        $scope.showSmscHistory = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/smsc/activity-history/smsc-activity-history.edr.history.html',
                ...
                resolve: {
                    smscHistoryEdrs: function (GeneralESService) {
                        return GeneralESService.findSmscHistoryEdrsRemote(edrRecord.cdrKey);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
    ...
    SmscActivityHistoryRemoteModule.controller('SmscActivityHistoryRemoteSummarizeCtrl', function ($scope, $log) {
        $log.debug('SmscActivityHistoryRemoteSummarizeCtrl');
