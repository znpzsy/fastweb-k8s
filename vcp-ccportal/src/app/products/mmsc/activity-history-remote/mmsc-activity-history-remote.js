(function () {
    ...
    angular.module('ccportal.products.mmsc.activity-history-remote', []);

    var MmscActivityHistoryRemoteModule = angular.module('ccportal.products.mmsc.activity-history-remote');

    MmscActivityHistoryRemoteModule.config(function ($stateProvider) {

        $stateProvider.state('products.mmsc.activity-history-remote', {
            abstract: true,
            url: "/activity-history-remote",
            templateUrl: "products/mmsc/activity-history/mmsc-activity-history.html"
        }).state('products.mmsc.activity-history-remote.edr', {
            abstract: true,
            url: "",
            templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.html',
            controller: 'MmscActivityHistoryRemoteCtrl'
        }).state('products.mmsc.activity-history-remote.edr.view', {
            url: "",
            views: {
                'permanentTable': {
                    templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.permanent.html'
                },
                'transientTable': {
                    templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.transient.html'
                }
            }
        });
    ...
    MmscActivityHistoryRemoteModule.controller('MmscActivityHistoryRemoteCtrl', function ($scope, $log, $q, $uibModal, $filter, $timeout, $controller, DateTimeConstants, GeneralESService,
                                                                              UtilService) {
        $log.debug('MmscActivityHistoryRemoteCtrl');
        ...
        $scope.reloadTable = function (tableParams) {
        ...
        var getSummarizeCounts = function () {
            ...
            GeneralESService.getMmscPermanentCountRemote(filter).then(function (response) {
                $log.debug("Found permanent record count: ", response);

                result.permanent = response.count;

                GeneralESService.getMmscTransientCountRemote(filter).then(function (response) {
                    $log.debug("Found transient record count: ", response);

                    result.transient = response.count;

                    GeneralESService.getMmscDeliveredCountRemote(filter).then(function (response) {
        ...
        $scope.throttledReloadTable = _.throttle(function () {
            $scope.reloadTable($scope.mmscPermanentEdrList.tableParams);
            $scope.reloadTable($scope.mmscTransientEdrList.tableParams);
        ...
        $scope.showDetails = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.details.html',
        ...
        $controller('MmscActivityHistoryRemotePermanentCtrl', {$scope: $scope});
        $controller('MmscActivityHistoryRemoteTransientCtrl', {$scope: $scope});
        $controller('MmscActivityHistoryRemoteHistoryCtrl', {$scope: $scope});
        $controller('MmscActivityHistoryRemoteSummarizeCtrl', {$scope: $scope});
    });

    MmscActivityHistoryRemoteModule.controller('MmscActivityHistoryRemotePermanentCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams, DateTimeConstants,
                                                                                       ReportingExportService, UtilService, GeneralESService, TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION) {
        $log.debug('MmscActivityHistoryRemotePermanentCtrl');

        $scope.permanentResultTemplateUrl = 'products/mmsc/activity-history/mmsc-activity-history.edr.result.tpl.html';
        ...
        var exportPermanentEdrs = function (mimeType) {
            ...
            var bodyPayload = GeneralESService.prepareMainEdrQueryPayload(filter, ['sender.address', 'recipient.address'], 'eventTime', additionalFilterFields, termFilterJSON);
            var bodyPayloadStr = JSON.stringify(bodyPayload).replace(/\+/g, '%2b');

            var srcUrl = '/mmsc-troubleshooting-remote-rest/v2/elastic-search/export';
        ...
        $scope.mmscPermanentEdrList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                GeneralESService.findMmscPermanentEdrsRemote(filter, additionalFilterFields).then(function (response) {
    ...
    MmscActivityHistoryRemoteModule.controller('MmscActivityHistoryRemoteTransientCtrl', function ($scope, $log, $q, $filter, notification, $translate, $uibModal, NgTableParams, UtilService, DateTimeConstants,
                                                                                       ReportingExportService, GeneralESService, MmscTroubleshootingService, TROUBLESHOOTING_RECORD_COUNT_LIMIT_FOR_NOTIFICATION) {
        $log.debug('MmscActivityHistoryRemoteTransientCtrl');

        $scope.transientResultTemplateUrl = 'products/mmsc/activity-history/mmsc-activity-history.edr.result.tpl.html';
        ...
        $scope.mmscTransientEdrList.tableParams = new NgTableParams({
            ...
            getData: function ($defer, params) {
                ...
                GeneralESService.findMmscTransientEdrsRemote(filter).then(function (response, additionalFilterFields) {
        ...
        $scope.showMessageContent = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.messagecontent.html',
                size: 'lg',
                resolve: {
                    contentIdList: function () {
                        var deferred = $q.defer();

                        MmscTroubleshootingService.getContentIdListRemote(edrRecord.messageID).then(function (response) {
                ...
                controller: function ($scope, $uibModalInstance, contentIdList) {
        ...
        var cancelOrSendImmediately = function (edr, isCancel) {
            ...
            modalInstance.result.then(function (message) {
                ...
                if (isCancel) {
                    ...
                    MmscTroubleshootingService.cancelMessageRemote(message.messageId, message.recipient).then(function (response) {
                        notification({
                            type: 'success',
                    ...
                    MmscTroubleshootingService.retryMessageRemote(message.messageId, message.recipient).then(function (response) {
                        ...
                        $log.debug('Cannot sending the message now: ', response);
                        notification({
                            type: 'warning',
                            text: response.data.message
                        });
    ...
    MmscActivityHistoryRemoteModule.controller('MmscActivityHistoryRemoteHistoryCtrl', function ($scope, $log, $filter, notification, $translate, $uibModal, NgTableParams, DateTimeConstants) {
        $log.debug('MmscActivityHistoryRemoteHistoryCtrl');
        ...
        $scope.showMmscHistory = function (edrRecord) {
            var modalInstance = $uibModal.open({
                animation: false,
                templateUrl: 'products/mmsc/activity-history/mmsc-activity-history.edr.history.html',
                ...
                resolve: {
                    mmscHistoryEdrs: function (GeneralESService) {
                        ...
                        return GeneralESService.findMmscHistoryEdrsRemote(edrRecord.messageID, recipientAddress);
                    },
                    edrRecord: function () {
                        return edrRecord;
                    },
    ...
    MmscActivityHistoryRemoteModule.controller('MmscActivityHistoryRemoteSummarizeCtrl', function ($scope, $log) {
        $log.debug('MmscActivityHistoryRemoteSummarizeCtrl');
