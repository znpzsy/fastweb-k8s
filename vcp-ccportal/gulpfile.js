...
var applicationName = 'vcp-ccportal';
...
// Copy .htaccess.
gulp.task('copy-htaccess', ['minify-view-htmls'], function () {
    return gulp.src([bases.src + '/.htaccess']).pipe(gulp.dest(bases.dist));
});

// Copy files to destination.
gulp.task('copy', ['copy-img', 'copy-i18n', 'copy-html5-js', 'copy-fonts', 'copy-htaccess']);
...
gulp.task('config', function () {
    return gulp.src('./src/app-config.json')
        .pipe(gulpNgConfig('ccportal.config', {
            //constants: {}
        }))
        .pipe(gulp.dest('./src/'))
});
    ...
    return connect.server({
        ...
        middleware: function (connect) {
            return [
                ...
                connect().use('/ccportal/fonts', serveStatic('bower_components/font-awesome/fonts')),
                connect().use('/ccportal/fonts', serveStatic('bower_components/bootstrap/fonts')),
                connect().use('/ccportal/fonts', serveStatic(bases.src + '/css/google-fonts')),
                connect().use('/ccportal/js', serveStatic(bases.src + '/html5')),
                connect().use('/ccportal/img', serveStatic(bases.src + '/img')),
                connect().use('/ccportal', serveStatic(bases.src + '/app')),
                connect().use('/ccportal', serveStatic(bases.tmp))
            ];
        }
    });
});
...
gulp.task('backup-the-old-portals', ['create-the-remote-directory'], function () {
    ...
    for (var i in remoteConfig) {
        ...
        var command = 'cp -r ' + remoteConfig[i].deploymentDirectory + ' ' + remoteConfig[i].deploymentDirectory + '-`date +"%Y-%m-%d-%dT%H-%M"`'; // Backup the old portal deployment.
...
gulp.task('extract-the-new-portal', ['clean-the-deployment-directory'], function () {
    ...
    for (var i in remoteConfig) {
        ...
        var command = 'tar xzf `ls -td ' + remoteConfig[i].tmpDirectory + '/' + applicationName + '-*.tar.gz | awk \'NR==1\'` -C ' + remoteConfig[i].deploymentDirectory + ' --warning=no-timestamp'; // Extract the new portal codes.
